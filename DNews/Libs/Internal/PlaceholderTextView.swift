...
class PlaceholderTextView: UITextView {
    ...
    func listenForTextChangedNotifications() {
        NSNotificationCenter.defaultCenter().addObserver(self, selector: #selector(PlaceholderTextView.textChangedForPlaceholderTextView(_:)), name:UITextViewTextDidChangeNotification , object: self)
        NSNotificationCenter.defaultCenter().addObserver(self, selector: #selector(PlaceholderTextView.textChangedForPlaceholderTextView(_:)), name:UITextViewTextDidBeginEditingNotification , object: self)
    }
    ...
    func placeholderBoundsContainedIn(containerBounds: CGRect) -> CGRect {
        ...
        var baseRect = UIEdgeInsetsInsetRect(containerBounds, UIEdgeInsetsMake(13, 10, 0, 0))
        ...
        if let paragraphStyle = typingAttributes[NSParagraphStyleAttributeName] as? NSParagraphStyle {
            baseRect.offsetInPlace(dx: paragraphStyle.headIndent, dy: paragraphStyle.firstLineHeadIndent)
        }
